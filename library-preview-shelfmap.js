...
/*function to append text to records where the RTA Location Code requires users to do something*/
 ...
 function AppendToHoldings() {
	  ...
	  var bibid = $(this).closest($('div.documentSummary')).attr('id').replace(/FETCH-rca_catalog_u/g,'').replace(.{1}$);	  	  	  	  
	  ...
	  if(invalidSMLocs.indexOf(Loc) == -1) {
	     // get bib id and append shelfmap url/link
	      $( "<span><a href='https://app.shelfmap.co.uk/fp/fp?icode=44RCA&id=" + bibid + "' target='_blank' class='ShelfMap_anchor' title='Show item on ShelfMap'><img src='xxx' alt='ShelMap drop pin' />Show me the book</a></span></br>").insertAfter( $(this) );
   	  }		  
          if (Loc == 'CRL') {
